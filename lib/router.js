...
module.exports = function(app, roomFactory){
  ...
  function getRoom(req, res, next){
    var found = roomFactory.getById(req.params['roomId']);
    if (found){
      req.room = found;
      next();
    }
  ...
  function remove(req, res){
    roomFactory.destroyRoom(req.room.id);
    res.send(204);
  }
  ...
  function create(req, res, next){
    ...
    var newRoom = roomFactory.create(req.body.config);
    req.room = newRoom;
  ...
  function createQueue(req, res){
    var room = roomFactory.queue(req.body.uid, req.body.config);
