...
var onRoomCreated = function(room){
  ...
  room.on('room:update', function(){
    roomsIO.in(this.id).emit('room:update', this.toJSON());
  });

  room.on('room:waiting', function(){
    roomsIO.in(this.id).emit('room:waiting', this.toJSON());
  });

  room.on('room:full', function(){
    roomsIO.in(this.id).emit('room:full', this.toJSON());
  });

  room.on('room:ready', function(){
    roomsIO.in(this.id).emit('room:ready', this.toJSON());
  });

  room.on('room:start', function(){
    roomsIO.in(this.id).emit('room:start', this.toJSON());
  });
