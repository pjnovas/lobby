...
Room.prototype.start = function(userId){

  if (this.owner !== 'system' && 
      this.owner !== 'queue' &&
      this.owner !== userId){
    throw new NotOwnerError('Only the owner can start this room');
  }
...
Room.prototype.update = function(props){
  var user = getUser(props.uid);

  if (props.uid !== this.owner){
    throw new NotOwnerError('Only the owner can update a room');
  }

  var news = _.omit(props, ['id', 'owner', 'status', 'uid']);
  _.extend(this, news);
};
